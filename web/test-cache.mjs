...
check('stamp-modules versions every import', () => {
  ...
  // One stamp per specifier, and no doubling. Count on the specifier list rather
  // than a raw substring count: `?v=` also appears in the wasm URL that init() is
  // given, so a global split over-counts.
  assert.equal(after.filter((s) => s.spec.includes('?v=TS1')).length, after.length,
    'not every specifier got the stamp');
  assert.ok(!stamped.includes('?v=TS1?v='), 'double-stamped URL produced');
